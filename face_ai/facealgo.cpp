...
void FaceAlgo::initFaceModels(std::string detect_model_path, std::string recog_model_path, std::string face_db_dir) {
    try {
        ...
        std::cerr << "Error loading face model file: " << e.what() << std::endl;
        // 进行错误处理，比如重新加载默认模型文件或者退出程序
    }
...
void FaceAlgo::detectFace(cv::Mat &image, std::vector<std::shared_ptr<faceInfo>> &infoList, bool showFPS) {
    if (faceDetector == nullptr) {
        std::cerr << "Error: faceDetector is not initialized" << std::endl;
        // 进行错误处理，比如退出函数或者抛出异常
        return;
    }
    cv::TickMeter tm;
...
void FaceAlgo::matchFace(cv::Mat &frame, std::vector<std::shared_ptr<faceInfo>> &infoList, bool l2) {
    if (faceRecognizer == nullptr) {
        std::cerr << "Error: faceRecognizer is not initialized" << std::endl;
        // 进行错误处理，比如退出函数或者抛出异常
        return;
    }
    double cosine_similar_thresh = 0.363;
    double l2norm_similar_thresh = 1.128;
    for(const auto& face : infoList) {
        ...
        for(const auto& item : this->face_models) {
